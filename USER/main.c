...
static char cmd_StaffLogin[]="user:";			//打卡指令
		
static char cmd_StaffSignup[]="user:,password:";//注册和查询指令

static char cmd_Exit[]="exit";					//退出指令
/*******************************************************************************/
...
/**********************************子函数定义*************************************/
void Project_InitAll();	//外设初始化
void DeviceStart(void);	//设备上电
void TimeSet(void);		//设置系统时间

uint8_t KEY_Scan(void);	//读取按键状态

void Enter_loginMeun(void);	//登记
void Enter_lookupMeun(void);//查询
void Enter_signupMeun(void);//注册

void staffSignupNew();
void staff_searchName(uint8_t searchMode);			//查找员工
void staffLoginNew_intoDatabase(uint8_t serialNum);	//录入员工登记信息

void Printf_StaffNotFound();						//输出“没有找到用户”并以2Hz闪烁LED5次
void Printf_logsAll(uint8_t serialNum);				//输出用户的所有打卡信息
void Printf_Loading(uint16_t delay_time);			//模拟加载
void Printf_MainMeun(uint8_t MainMeun_Option);		//打印主菜单
/*********************************************************************************/
...
int main(void)
{
	...
	while (1)
	{
		...
		switch(KEY_Scan())
			{
				case 0:
					Enter_loginMeun(); break;
			
				case 1:
					Enter_lookupMeun();	break;
				
				case 2:
					Enter_signupMeun(); break;
